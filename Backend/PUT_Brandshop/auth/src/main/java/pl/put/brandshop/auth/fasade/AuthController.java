package pl.put.brandshop.auth.fasade;
...
import pl.put.brandshop.auth.services.UserService;
...
public class AuthController
{
    ...
    public ResponseEntity<AuthResponse> setRole(@RequestBody SetRoleData setRoleData)
    {
        return userService.setRole(setRoleData);
    }

    @RequestMapping(path="/set-lock",method = RequestMethod.PATCH)
    public ResponseEntity<AuthResponse> setRole(@RequestBody SetLockData setLockData)
    {
    ...
    public ResponseEntity<AuthResponse> authorize(HttpServletRequest request,HttpServletResponse response) {
        try{
            ...
            return ResponseEntity.status(401).body(new AuthResponse(Code.A7));
        }
    }
